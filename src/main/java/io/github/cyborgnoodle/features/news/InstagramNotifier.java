            ...
            for(String name : names){
                ...
                for(Media media : mlist){
                    ...
                    if(media.createdTime>NOTBEFORE){
                        ...
                        Boolean seen = noodle.news.registry.isSeen(media.id);

                        if(!seen){

                            noodle.news.registry.setSeen(media.id);

                            if(media.type.equalsIgnoreCase("image")){

                                ServerChannel c;
                                if(name.equalsIgnoreCase("cyborgnoodlebot")){
                                    c = ServerChannel.SPAM;
                                }
                                ...
                                if(!noodle.isTestmode()) noodle.getChannel(c).sendMessage(msg);

                                if(noodle.reddit.isConnected() && !noodle.isTestmode()){
                                    noodle.
                                            reddit.
